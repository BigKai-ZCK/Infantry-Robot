...
void JetsonComm_Control(UART_HandleTypeDef *huart)
{
    ...
    else if (DataRecFromJetson.ShootMode == RequestTrans)
    {
          ...
          DataSendToJetson.NeedMode = State_Reg.Shoot_Mode;
			...
			/** 记录当前Pitch、Yaw角度,并发送 **/
			  ...
			  float yaw_angle = YAW_ANGLE;
				...
				JetsonFlag[Record_Seq].CurAngle_Pitch = PITCH_ANGLE;
        ...
        Pitch.Jetson_TargetAngle = JetsonFlag[Jetson_Seq].CurAngle_Pitch +DataRecFromJetson.TargetPitchAngle ;
			
        Yaw.Jetson_TargetAngle = JetsonFlag[Jetson_Seq].CurAngle_Yaw - DataRecFromJetson.TargetYawAngle ;
        ...
        Jetson_VelocityYaw = JetsonFlag[Jetson_Seq].Velocity_Yaw;
			...
			{
				if(!ShootController.Jetson_ShootFlag && !StirMotor.PullTrigger)//检测到上一次pulltrigger已有赋值
