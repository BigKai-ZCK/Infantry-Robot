...
void mpu_get_data()
{
	...
	imu.ax = mpu_data.ax * BMI088_ACCEL_SEN;
	imu.ay = mpu_data.ay * BMI088_ACCEL_SEN;
	imu.az = mpu_data.az * BMI088_ACCEL_SEN;
	imu.mx = mpu_data.mx * MAG_SEN;
	imu.my = mpu_data.my * MAG_SEN;
	imu.mz = mpu_data.mz * MAG_SEN;	
...
void mpu_offset_call(void)
{
	...
	mpu_data.ax_offset /= 800;
	mpu_data.ay_offset /= 800;
	mpu_data.az_offset /= 800;
	mpu_data.gx_offset /= 800;
	mpu_data.gy_offset /= 800;
	mpu_data.gz_offset /= 800;
}
...
{
	/* yaw    -pi----pi */
	imu.yaw = -atan2(2 * q1 * q2 + 2 * q0 * q3, -2 * q2 * q2 - 2 * q3 * q3 + 1) * 57.295779513f;
	/* pitch  -pi/2----pi/2 */
	imu.pit = -asin(-2 * q1 * q3 + 2 * q0 * q2) * 57.3;
	/* roll   -pi----pi  */
	imu.rol = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2 * q2 + 1) * 57.295779513f;
}
