...
/** Macro Definition **/
  ...
  #define MEMORYRESET 2
	...
	#define SBUS_FRAME_LEN       25U         //Length of received data per frame
	#define BUFFER_LEN           30U          //length of a buffer
	#define RC_CH_MAX_RELATIVE 660.0f       	
...
//	#define RC_CH_VALUE_MAX ((uint16_t)1684)
	...
	#define RC_CH3    ((uint8_t)3)
	#define RC_CH4    ((uint8_t)4)
  #define RC_CH5    ((uint8_t)5)

	/** RC Switch Config**/	
	#define RC_SW_Right ((uint8_t)0)
	#define RC_SW_Left  ((uint8_t)1)
	#define RC_SW_Right_F ((uint8_t)2)
	#define RC_SW_Left_F  ((uint8_t)3)
	
	/*becasue of the design of MARKII remote controol, I suggest using FRT,BAK,MID to replace  UP, DOWN,MID  */
	
	#define RC_SW_UP    ((uint8_t)4)	
	#define RC_SW_DOWN  ((uint8_t)2)
	#define RC_SW_MID   ((uint8_t)0)
	
	#define RC_SW_FRT   ((uint8_t)4)	
	#define RC_SW_BAK   ((uint8_t)2)
	#define RC_SW_MID   ((uint8_t)0)
	
	
  #define RC_BT_PRE  ((uint8_t) 2)
	#define RC_BT_UNP  ((uint8_t) 0)
...
//  uint8_t  Switch_Right;
  uint8_t Switch_Left_F;
	uint8_t Switch_Right_F;
	uint8_t Button_Left;
...
//	int16_t Get_Channel_Val(RCDecoding_Type *RC_ReceiveData,uint8_t channel_num);
//	uint8_t Get_Switch_Val(RCDecoding_Type *RC_ReceiveData,uint8_t switch_num);
  uint8_t Get_Button_Val(RCDecoding_Type *RC_ReceiveData,uint8_t button_num);
