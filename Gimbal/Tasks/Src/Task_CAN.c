...
/**Variable Definition**/
	...
	uint16_t CurrentTick_CAN, LastTick_CAN, TaskTime_CAN;
...
void Task_CAN(void *parameters)
{
    ...
    while(1)
    {
			...
			LastTick_CAN = CurrentTick_CAN;
			CurrentTick_CAN = HAL_GetTick();
			TaskTime_CAN = CurrentTick_CAN - LastTick_CAN;
    }
}
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    switch(RxHeader.StdId)                                             
    {
				case 0x208:       
				{					
						Pitch.FrameCounter++;                                        /*PITCH轴电机帧计数增加*/
						Pitch.Mechanical_Angle = aData[0] << 8 | aData[1];           /*PITCH轴电机机械角*/
						Pitch.Mechanical_Speed = aData[2] << 8 | aData[3];           /*PITCH轴电机机械速度*/
						Pitch.Torque_Current_Real = aData[4] << 8 | aData[5];        /*PITCH轴电机真实电流*/
						Pitch.MotorTemp = aData[6];                                  /*PITCH轴电机温度*/
						break;
				}			
...
void HAL_CAN_RxFifo1MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    switch(RxHeader.StdId)
		{                                  
			...
			case 0x205: 
			{				
				FricMotor_Left.FrameCounter++;                                 
				FricMotor_Left.Mechanical_Angle = aData[0] << 8 | aData[1];
				FricMotor_Left.RealSpeed = aData[2] << 8 | aData[3];
				FricMotor_Left.RealCurrent = aData[4] << 8 | aData[5];
				break;
			}
			case 0x206:
			{
				FricMotor_Right.FrameCounter++;                                 
				FricMotor_Right.Mechanical_Angle = aData[0] << 8 | aData[1];
				FricMotor_Right.RealSpeed = aData[2] << 8 | aData[3];           
			...
			{				
					...
					StirMotor.Mechanical_Angle_Last = StirMotor.Mechanical_Angle;   
					StirMotor.Mechanical_Angle = aData[0] << 8 | aData[1];           
					StirMotor.RealSpeed = aData[2] << 8 | aData[3];                 
					StirMotor.RealCurrent = aData[4] << 8| aData[5];                 
					delta_angle = delta_Mechanical_angle(StirMotor.RealSpeed,\
																							StirMotor.Mechanical_Angle_Last,\
																							StirMotor.Mechanical_Angle); /*拨盘机械角两次差值*/
					StirMotor.Mechanical_Angle_Total += delta_angle;                 /*机械角差值积分*/
					StirMotor.RealAngle_total = StirMotorChangeToAbsAngle(StirMotor.Mechanical_Angle_Total);/*将机械角转为真实角度*/
					break;    									
			}
	}
}
