...
uint8_t RCBuffer[2][BUFFER_LEN];
int16_t channel[16];
uint8_t RC_Rx_Mem;

extern uint8_t use_image_transmit_flag;
 ...
 * @note: 
 */ 
uint8_t RC_UART_IRQHandler(UART_HandleTypeDef *huart)
{ 
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;                   

    static uint8_t this_time_rx_len = 0;
	
	  static uint8_t complete_transmit_framecounter = 0;
    ...
    if (__HAL_UART_GET_IT_SOURCE(huart, UART_IT_IDLE) != RESET)
    {
      ...
      this_time_rx_len = (BUFFER_LEN) - __HAL_DMA_GET_COUNTER(&hdma_usart3_rx);
TaskRCtest=this_time_rx_len;
...
//      if (this_time_rx_len != SBUS_FRAME_LEN)
//        RC_Rx_Mem = MEMORYRESET;
//      else
//			{

//			}				
			complete_transmit_framecounter++;
...
//		RC_Rx_Mem = MEMORY0;
		return complete_transmit_framecounter;
...
void SBUS_Data_Update()
{
	if (RC_Rx_Mem == MEMORY0)
  {
		SBUS_Decode(RCBuffer[0],channel);
		SBUS_To_RC(channel, &RC_ReceiveData);
  }
  else if (RC_Rx_Mem == MEMORY1)
	{
		SBUS_Decode(RCBuffer[1],channel);
		SBUS_To_RC(channel, &RC_ReceiveData);
	}
 ...
 * @note: 
 */
void SBUS_Decode(uint8_t *buffer, int16_t *ch)
{
  
	ch[0]  = ((int16_t)(buffer[1]    |buffer[2]<<8)                 & 0x07FF);         //右摇杆 左右
	ch[1]  = ((int16_t)(buffer[2]>>3 |buffer[3]<<5)                 & 0x07FF);         //右摇杆 前后
	ch[2]  = ((int16_t)(buffer[3]>>6 |buffer[4]<<2 |buffer[5]<<10)  & 0x07FF);         //左摇杆 前后
	ch[3]  = ((int16_t)(buffer[5]>>1 |buffer[6]<<7)                 & 0x07FF);         //左摇杆 左右
	ch[4]  = ((int16_t)(buffer[6]>>4 |buffer[7]<<4)                 & 0x07FF);         //左前拨杆  E  向后拨为1810 中间为992 向前为172
	ch[5]  = ((int16_t)(buffer[7]>>7 |buffer[8]<<1 |buffer[9]<<9)   & 0x07FF);         //右前拨杆  F
	ch[6]  = ((int16_t)(buffer[9]>>2 |buffer[10]<<6)                & 0x07FF);         //左拨杆  B
	ch[7]  = ((int16_t)(buffer[10]>>5|buffer[11]<<3)                & 0x07FF);         //右拨杆  C
	ch[8]  = ((int16_t)(buffer[12]   |buffer[13]<<8)                & 0x07FF);         //左按键  A   不按为172 按下为1810
	ch[9]  = ((int16_t)(buffer[13]>>3|buffer[14]<<5)                & 0x07FF);         //右按键  D
	ch[10] = ((int16_t)(buffer[14]>>6|buffer[15]<<2|buffer[16]<<10) & 0x07FF);         //左拨轮       前1810， 后172
	ch[11] = ((int16_t)(buffer[16]>>1|buffer[17]<<7)                & 0x07FF);         //右拨轮
	ch[12] = ((int16_t)(buffer[17]>>4|buffer[18]<<4)                & 0x07FF);
	ch[13] = ((int16_t)(buffer[18]>>7|buffer[19]<<1|buffer[20]<<9)  & 0x07FF);
	ch[14] = ((int16_t)(buffer[20]>>2|buffer[21]<<6)                & 0x07FF);
	ch[15] = ((int16_t)(buffer[21]>>5|buffer[22]<<3)                & 0x07FF);
 ...
 * @param {none} 
 * @return: void
 * @note: convert the data from SBUS to DBUS data, so that it can fit prevenient code. 
					You can also change the meaning of channel in remote control, but I do not suggeest it.
 */
void SBUS_To_RC(int16_t *ch, RCDecoding_Type *RD)
{
	RD->ch0 = ch[0];    //左右
	RD->ch1 = ch[1];    //前后
	RD->ch2 = ch[3];    //yaw
	...
	RD->ch4 = ch[10];
	RD->ch5 = ch[11];
	RD->Switch_Left = ch[6]& 0x06;
  RD->Switch_Right = ch[7] & 0x06;
  RD->Switch_Left_F = ch[4] & 0x06;
	RD->Switch_Right_F = ch[5] &0x06;
	RD->Button_Left = ch[8] & 0x02;
	RD->Button_Right = ch[9] & 0x02;
//	
}
...
//	
//	if((BoardContent.game_progress == GAME_CHECK) || (BoardContent.game_progress == GAME_DOWNCOUNT))
//	{
//		State_Reg.Ctrl_Mode = Ctrl_Protect;
//	}
//	
//	if(BoardContent.game_progress == GAME_ONGOING)
//	{
//		LASER_OFF();
//	}
//	
//	StateSend.Trigger_Auto = (Get_Switch_Val(&LastRC_ReceiveData, RC_SW_Right) == RC_SW_MID) & (Get_Switch_Val(&RC_ReceiveData, RC_SW_Right) == RC_SW_DOWN);//为了让云台切自动的时候不动
//	
...
//			StateSend.Trigger_Stir = (Get_Switch_Val(&LastRC_ReceiveData, RC_SW_Left) == RC_SW_MID) & (Get_Switch_Val(&RC_ReceiveData, RC_SW_Left) == RC_SW_DOWN);
...
//			StateSend.Trigger_Fric = (Get_Switch_Val(&LastRC_ReceiveData, RC_SW_Left) == RC_SW_MID) & (Get_Switch_Val(&RC_ReceiveData, RC_SW_Left) == RC_SW_UP);
//			StateSend.Stir_On = (Get_Switch_Val(&RC_ReceiveData, RC_SW_Left) == RC_SW_DOWN);			
//			StateSend.Reserved = (Get_Switch_Val(&RC_ReceiveData, RC_SW_Left) == RC_SW_UP);//保底左拨上右拨下进陀螺
//			
//		}
//	}
...
void RC_Data_Update(void)
{
  ...
  if(use_image_transmit_flag == 0)
	...
	{
			RC_ReceiveData.mouse.x = ext_video_transmitter.mouse_x;                                         // Mouse X axis
			RC_ReceiveData.mouse.y = ext_video_transmitter.mouse_y;                                         // Mouse Y axis
			RC_ReceiveData.mouse.z = ext_video_transmitter.mouse_z;                                       // Mouse Z axis
                                                                                                              
			RC_ReceiveData.mouse.press_left  = ext_video_transmitter.left_button_down;                                                   // Mouse Left Pressed
			RC_ReceiveData.mouse.press_right = ext_video_transmitter.right_button_down;                                                      // Mouse Right Pressed
			RC_ReceiveData.key_board.key_code = ext_video_transmitter.keyboard_value;             
	}		// KeyBoard value
...
void RC_Data_Decode(void)
{
	...
	if(Get_Keyboard_Val(&RC_ReceiveData, KEY_A) && Get_Keyboard_Val(&RC_ReceiveData, KEY_D) && Get_Keyboard_Val(&RC_ReceiveData, KEY_W)\
	&& Get_Keyboard_Val(&RC_ReceiveData, KEY_S))
	lost_time++;
	else lost_time = 0;
	if(lost_time > 30)
	{	
    RC_ReceiveData.Switch_Left = RC_SW_DOWN;                     
	...
	switch (State_Reg.Ctrl_Mode)
	{
		...
		{			
			...
			State_Reg.V_Pressed					= Get_Keyboard_Val(&RC_ReceiveData, KEY_V);
			State_Reg.B_Pressed         = Get_Keyboard_Val(&RC_ReceiveData, KEY_B);
			...
			StateSend.Trigger_Shift			= (~Get_Keyboard_Val(&LastRC_ReceiveData, KEY_SHIFT)) & (Get_Keyboard_Val(&RC_ReceiveData, KEY_SHIFT));
